...
internal sealed class Modulo<TNumber> : DiceExpression<TNumber> where TNumber : INumber<TNumber>
{
    internal DiceExpression<TNumber> Left { get; }
    internal DiceExpression<TNumber> Right { get; }

    internal Modulo(DiceExpression<TNumber> left, DiceExpression<TNumber> right)
    {
        Left = left;
        Right = right;
    }
    ...
    {        
        ...
        {
            return new EvaluatorError("Evaluator.ModuloByZero", "Modulo by zero detected!");
        }
